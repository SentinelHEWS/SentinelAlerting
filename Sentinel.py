...
RMUYALTO = gpd.read_file("XXX.shp") #SHPs de AGEBs con peligro muy alto por temperaturas máximas urbanas
RALTO = gpd.read_file("XXX.shp")
...
refU = db.reference("XXX")
refA = db.reference("XXX")
...
def sentinelgorithm(pesoPersona, alturaPersona, edad, pobRA, Tambuser):
    ...
    if pobRA == "true":
        Ssurv = 0.55 * pesoPersona #W/kg OBTENIDO DE BALANCE TÉRMICO IDEALIZADO A TRES HORAS (Q = mCpDeltaT)
    else:
...
#∘₊ ☆─── SECCIÓN 4 ───☆₊∘#
# ALERTAMIENTO PUSH
def sentinelAlerts(osID, titulo, mensaje, acID):
    if not osID: return
...
def alertICA(tituloAire, mensajeAire):
    headers = {
        "Authorization": f"Basic {ONESIGNAL_REST_API_KEY}",
        "Content-Type": "application/json; charset=utf-8"
    }
    ...
    payload = {
        ...
        "android_channel_id": "7dc7f185-2733-43f1-b5e5-f6d3be9fe7b4"
    }
...
if datosUsuarios:
    ...
    for aUUID, data in datosUsuarios.items():
        ...
        if lat is not None and long is not None:
            ...
            if RMUYALTO.geometry.contains(pusuario).any():
                Tambuser += 3
            elif RALTO.geometry.contains(pusuario).any():
                Tambuser += 1.5
    ...
    if redAlert_aUUIDs:
        sentinelAlerts(
            ...
            "ANDROID_CHANNEL_ID_DE_ONESISGNAL"
        )
...
if ICA in ["MALO", "MUY MALO", "EXTREMADAMENTE MALO"]:
...
if aireSucio == True:
    ...
    alertICA(
        "🍃 AIRE LIMPIO", 
        "Gracias por tu cooperación. El estado del aire es seguro nuevamente."
    )
